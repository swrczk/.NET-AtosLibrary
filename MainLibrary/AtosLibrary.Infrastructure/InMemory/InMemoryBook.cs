...
{
    public class InMemoryBook : IInMemoryDb<BookEntity>
    {
        private readonly List<BookEntity> _bookEntities;

        public InMemoryBook()
        {
            _bookEntities = BookData();
        }

        public BookEntity Get(Guid id)
        {
            return _bookEntities.FirstOrDefault(x => x.Id == id);
        }
